...
class ControllerShippingBoxnow extends Controller {
	...
	public function index() {
		...
		$data['entry_partner_phone'] 		= $this->language->get('entry_partner_phone');
		...
		if (isset($this->request->post['boxnow_partner_name'])) {
			$data['boxnow_partner_name'] = $this->request->post['boxnow_partner_name'];
		} else {
			$data['boxnow_partner_name'] = $this->config->get('boxnow_partner_name');
		}

		if (isset($this->request->post['boxnow_partner_email'])) {
			$data['boxnow_partner_email'] = $this->request->post['boxnow_partner_email'];
		} else {
			$data['boxnow_partner_email'] = $this->config->get('boxnow_partner_email');
		}
		
		if (isset($this->request->post['boxnow_partner_phone'])) {
			$boxnow_partner_phone = $this->request->post['boxnow_partner_phone'];
			...
			if (preg_match($pattern, $boxnow_partner_phone)) {
				$data['boxnow_partner_phone'] = $boxnow_partner_phone;
			} else {
				// If the phone number does not match the pattern, do not save it
				$data['boxnow_partner_phone'] = '';
				// Optionally, you can provide an error message or handle the situation as needed
			}
		} else {
			$data['boxnow_partner_phone'] = $this->config->get('boxnow_partner_phone');
		}
	...
	public function deliveryRequests() {
		if (isset($this->request->get['order_id']) && $this->request->get['order_id']) {
			...
			$data = array(
				...
				"allowReturn" 			=> true,
				"origin" 				=> array (
					"contactName" => $this->config->get('boxnow_partner_name'),
					"contactEmail" => $this->config->get('boxnow_partner_email'),
					"contactNumber" => $this->config->get('boxnow_partner_phone'),
